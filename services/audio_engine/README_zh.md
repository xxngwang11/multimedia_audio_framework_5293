...
**图1** 引擎架构图
![](../../figures/zh-cn_image_proaudio.png)
...
**图2** 下行播放数据流转以及节点连接图
![](../../figures/zh-cn_image_proaudio_renderer.png)

**图3** 上行录制数据流转以及节点连接图
![](../../figures/zh-cn_image_proaudio_capturer.png)
...
而上行需要遍历所有SourceOutPutNode，对每一个节点都执行拉数据操作（同样为递归式），但ProAudio节点具有分发功能，因此，只有第一个节点可以触发上一个节点拉数据，其他节点均取到第一个节点所拉到的数据，并不会造成重复拉数据。
