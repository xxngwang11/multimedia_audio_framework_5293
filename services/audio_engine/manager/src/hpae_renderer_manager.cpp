...
int32_t HpaeRendererManager::CreateInputSession(const HpaeStreamInfo &streamInfo)
{
    ...
    if (sinkInfo_.lib == "libmodule-split-stream-sink.z.so") {
        ...
        nodeInfo.sceneType = TransStreamTypeToSceneType(streamInfo.streamType, isCollaborationEnabled_);
    }
    if (IsMchDevice()) {
    ...
    auto request = [this, isCollaborationEnabled]() {
        ...
        if (isCollaborationEnabled_ && hpaeCoBufferNode_ == nullptr) {
            ...
            for (auto it : sessionNodeMap_) {
                ...
                HandleCollaborationStateChangedInner(TransStreamTypeToSceneType(nodeInfo.second.sceneType,
