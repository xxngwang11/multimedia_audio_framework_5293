...
protected:
    void SetUp() override {
        ...
        nodeInfo_.frameLen = 960;
        nodeInfo_.channels = STEREO;
        nodeInfo_.format = SAMPLE_F32LE;
        nodeInfo_.sessionId = DEFAULT_SESSION_ID01;
    }
...
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
...
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
    HpaeNodeInfo nodeInfo = nodeInfo_;
...
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
    auto preNode = std::make_shared<HpaeSinkInputNode>(nodeInfo_);
    cluster->Connect(preNode);
    EXPECT_GT(cluster->GetConnectSinkInputNum(), 1);
...
HWTEST_F(HpaeVirtualProcessClusterTest, ResetAllClearsAllConnections, TestSize.Level0)
{
    ...
    HpaeNodeInfo info1 = nodeInfo_;
    info1.sessionId = DEFAULT_SESSION_ID01;
    HpaeNodeInfo info2 = nodeInfo_;
    info2.sessionId = DEFAULT_SESSION_ID02;
    
    auto preNode1 = std::make_shared<HpaeSinkInputNode>(info1);
    auto preNode2 = std::make_shared<HpaeSinkInputNode>(info2);
...
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
    EXPECT_NE(cluster->GetSharedInstance()!= nullptr, true);
    EXPECT_EQ(cluster->GetSharedInstance()->GetNodeId(), nodeInfo_.nodeId);
}

HWTEST_F(HpaeVirtualProcessClusterTest, GetOutputPortReturnsMixerPort, TestSize.Level0)
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
    EXPECT_NE(cluster->GetOutputPort() != nullptr, true);
}

HWTEST_F(HpaeVirtualProcessClusterTest, ConnectCreatesGainAndConverterNodes, TestSize.Level0)
{
    std::shared_ptr<HpaeVirtualProcessCluster> cluster = std::make_shared<HpaeVirtualProcessCluster>(nodeInfo_);
    HpaeNodeInfo preNodeInfo = nodeInfo_;
    preNodeInfo.sessionId = DEFAULT_SESSION_ID01;
    auto preNode = std::make_shared<HpaeSinkInputNode>(preNodeInfo);
    cluster->Connect(preNode);
