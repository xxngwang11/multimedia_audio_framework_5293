    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> selectedDesc)
{
    CHECK_AND_RETURN_RET_LOG(audioRendererFilter != nullptr && !selectedDesc.empty() && selectedDesc[0] != nullptr,
        ERROR, "ptr exception");
    SelectOutputDeviceLog(audioRendererFilter, selectedDesc);
    ...
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> selectedDesc)
{
    CHECK_AND_RETURN_RET_LOG(audioCapturerFilter != nullptr && !selectedDesc.empty() && selectedDesc[0] != nullptr,
        ERROR, "ptr exception");
    SelectInputDeviceLog(audioCapturerFilter, selectedDesc);
...
void AudioRecoveryDevice::SelectInputDeviceLog(sptr<AudioCapturerFilter> audioCapturerFilter,
    std::vector<std::shared_ptr<AudioDeviceDescriptor>> selectedDesc)
{
