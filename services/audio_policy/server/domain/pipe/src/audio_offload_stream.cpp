...
void AudioOffloadStream::SetOffloadStatus(OffloadAdapter offloadAdapter, uint32_t sessionId)
{
    ...
    HILOG_COMM_INFO("Set offload session: %{public}u", sessionId);
...
{
    HILOG_COMM_INFO("Set offload enable for stream: %{public}u", sessionId);
...
{
    HILOG_COMM_INFO("Unset offload enable for stream: %{public}u", sessionId);
