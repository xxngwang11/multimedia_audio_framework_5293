    ...
    std::vector<std::shared_ptr<AudioStreamDescriptor>> &streamDescs)
{
    ...
    for (auto &streamDesc : streamDescs) {
        ...
        std::vector<std::string> busAddresses =
            AudioBusSelector::GetBusSelector().GetBusAddressesByStreamDesc(streamDesc);
        configManager_.GetStreamPropInfo(streamDesc, streamPropInfo, busAddresses);
#else
        configManager_.GetStreamPropInfo(streamDesc, streamPropInfo);
#endif
...
#ifdef CAR_AUDIO_DETECT
        std::string portNameA = AudioPolicyUtils::GetInstance().GetSinkName(deviceDescA, streamDescA->sessionId_);
#else
...
#ifdef CAR_AUDIO_DETECT
            std::string portNameB = AudioPolicyUtils::GetInstance().GetSinkName(deviceDescB, streamDescB->sessionId_);
#else
            AudioPipeType pipeTypeB = GetPipeType(streamDescB->routeFlag_, streamDescB->audioMode_);
            std::string portNameB =
                AudioPolicyUtils::GetInstance().GetSinkPortName(deviceDescB->deviceType_, pipeTypeB);
#endif
