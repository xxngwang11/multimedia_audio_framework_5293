...
int32_t AudioRenderer::CheckMaxRendererInstances()
{
    ...
    CHECK_AND_CALL_RET_FUNC(audioRendererChangeInfos.size() < static_cast<size_t>(maxRendererInstances), ERR_OVERFLOW,
...
int32_t AudioRendererPrivate::InitAudioStream(AudioStreamParams audioStreamParams)
{
    ...
    CHECK_AND_CALL_RET_FUNC(!ret, ret, HILOG_COMM_ERROR("[InitAudioStream]SetParams SetAudioStreamInfo Failed"));

    ret = GetAudioStreamIdInner(sessionID_);
    CHECK_AND_CALL_RET_FUNC(!ret, ret, HILOG_COMM_ERROR("[InitAudioStream]GetAudioStreamId err"));
...
bool AudioRendererPrivate::Start(StateChangeCmdType cmdType)
{
    ...
    CHECK_AND_CALL_RET_FUNC((state == RENDERER_PREPARED) || (state == RENDERER_STOPPED) || (state == RENDERER_PAUSED),
        false, HILOG_COMM_ERROR("[Start]Start failed. Illegal state:%{public}u", state));

    CHECK_AND_CALL_RET_FUNC(!isSwitching_, false,
...
bool AudioRendererPrivate::Pause(StateChangeCmdType cmdType)
{
    ...
    RendererState state = GetStatusInner();
    CHECK_AND_CALL_RET_FUNC(state == RENDERER_RUNNING, false,
...
void AudioRendererPrivate::SetSilentModeAndMixWithOthers(bool on)
{
    ...
    if (audioStream_->GetSilentModeAndMixWithOthers() && !on) {
        ...
        if (static_cast<RendererState>(audioStream_->GetState()) == RENDERER_RUNNING) {
            ...
            CHECK_AND_CALL_FUNC(ret == SUCCESS,
...
bool AudioRendererPrivate::SetSwitchInfo(IAudioStream::SwitchInfo info, std::shared_ptr<IAudioStream> audioStream)
{
    ...
    CHECK_AND_CALL_RET_FUNC(res == SUCCESS, false, HILOG_COMM_ERROR("[SetSwitchInfo]SetAudioStreamInfo failed"));
...
void AudioRendererPrivate::SetAudioHapticsSyncId(int32_t audioHapticsSyncId)
{
    ...
    CHECK_AND_CALL_FUNC(audioStream_ != nullptr,
    ...
    AudioStreamType audioStreamType)
{
    ...
    CHECK_AND_CALL_RET_FUNC(audioStream_ != nullptr, ERR_INVALID_PARAM,
