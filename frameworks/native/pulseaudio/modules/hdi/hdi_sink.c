...
static pa_resampler *UpdateResamplerIchannelMap(const char *sinkSceneType, struct Userdata *u)
{
    ...
    if ((!pa_channel_map_equal(pa_resampler_input_channel_map(resampler), &ichannelmap))) {
        AUDIO_INFO_LOG("UpdateResampler: sceneType [%{public}s], input channelnum: [%{public}d],",
        ...
        AUDIO_INFO_LOG("UpdateResampler: sceneType [%{public}s], input sample rate:[%{public}d], format:[%{public}d]",
            (char *)sinkSceneType, ispec.rate, ispec.format);
...
static void UpdateSceneToResamplerMap(pa_hashmap *sceneToResamplerMap, pa_hashmap *sceneToCountMap, pa_sink *si)
{
    ...
    while ((pa_hashmap_iterate(sceneToCountMap, &count, &sceneType))) {
        ...
        if (resampler == NULL) {
            // for now, use sample_spec from sink
            AUDIO_INFO_LOG("ResamplerMap new sceneType: [%{public}s], output channels [%{public}d]",
                (char *)sceneType, sink_spec.channels);
            AUDIO_INFO_LOG("ResamplerMap new [%{public}s], output sample rate [%{public}d], format: [%{public}d]",
                (char *)sceneType, sink_spec.rate, sink_spec.format);
            resampler = pa_resampler_new(
                si->core->mempool,
                &sink_spec, &sink_channelmap,
                &sink_spec, &sink_channelmap,
            ...
            if (!pa_sample_spec_equal(pa_resampler_output_sample_spec(resampler), &sink_spec) ||
                ...
                AUDIO_INFO_LOG("ResamplerMap: sceneType [%{public}s], new output channels [%{public}d]",
                    (char *)sceneType, sink_spec.channels);
                AUDIO_INFO_LOG("ResamplerMap: [%{public}s], new output sample rate %{public}d, format: [%{public}d]",
                    (char *)sceneType, sink_spec.rate, sink_spec.format);
...
static void SinkRenderPrimaryProcess(pa_sink *si, size_t length, pa_memchunk *chunkIn)
{
    ...
    while ((pa_hashmap_iterate(u->sceneToCountMap, &state, &sceneType))) {
        ...
        PrimaryEffectProcess(u, sinkSceneType);
        pa_memblock_release(chunkIn->memblock);
    }
...
static int32_t ProcessRenderUseTimingOffload(struct Userdata *u, bool *wait, int32_t *nInput, int32_t *writen)
{
    ...
    *nInput = (int32_t)nInputs;
    ...
    if ((fadingFlag != NULL) && (!strcmp(fadingFlag, "1"))) {
